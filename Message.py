                ...
                if statsu_keyword:
                    ...
                    if statsu_title:
                        ...
                        if statsu_data:
                            ...
                            await ctx.send(self.return_list_info(ctx.author.id, ctx.author.name))

    def return_list_info(self, id, name):
        text = f"ข้อมูลของ '{name}' ทั้งหมด:"
        text += "\n\n***Keyword***"
        for keyword in self.database["user"][str(id)]:
            text += f"\n[ {str(keyword) }]"
            for title in self.database["user"][str(id)][str(keyword)]:
                text += f"\n- {str(title)} {self.database['user'][str(id)][str(keyword)][str(title)]}"
        return text
