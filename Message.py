...
class Message(commands.Cog):
    ...
    async def install(self, ctx):
        if str(ctx.guild.id) in self.database["server"]:
            await ctx.send(f"เคยลงทะเบียนกับเซิฟเวอร์ '{ctx.guild.name}' อยู่แล้ว")
        else:
            ...
            await ctx.send(f"ลงทะเบียนสำหรับเซิฟเวอร์ {ctx.guild.name} ที่ห้อง '{channel}' เสร็จเรียบร้อยแล้ว")

    @commands.command()
    async def uninstall(self, ctx):
        if str(ctx.guild.id) not in self.database["server"]:
            await ctx.send(f"ไม่เคยลงทะเบียนกับเซิฟเวอร์ '{ctx.guild.name}' อยู่แล้ว")
        else:
            del self.database["server"][str(ctx.guild.id)]
            await ctx.send(f"ยกเลิกลงทะเบียนสำหรับเซิฟเวอร์ {ctx.guild.name} เรียบร้อยแล้ว")

    @commands.command()
    @commands.check(channel_only)
    async def condition(self, ctx):
        await ctx.send("เงื่อนไขการใช้งานคือการใช้งานโดยมี ';;' เป็นตัวเชื่อม")
                    ...
                    for word in self.database["user"][str(ctx.author.id)]:
                        for title in self.database["user"][str(ctx.author.id)][str(word)]:
                            if title == {}:
                                statsu_title = True
                    if statsu_title:
                        await ctx.send("ไม่พบข้อมูลของ 'title' โปรดใส่ข้อมูลก่อน")
                    else:
                        statsu_data = False
                        for word in self.database["user"][str(ctx.author.id)]:
                            for title in self.database["user"][str(ctx.author.id)][str(word)]:
    ...
    async def title_list(self, ctx, text):
        ...
        if value_userid == "user:False":
            ...
            await ctx.send(f"คุณไม่เคยลงคำว่า '{str(text)}' ใน keyword เลย")
        else:
    ...
    async def title_add(self, ctx, *, text):
        if ";;" not in text:
            await ctx.send("คำสั่งใช้งานผิด โปรดสั่งให้ครบเงื่อนไขด้วย")
            await ctx.send("ใช้คำสั่ง 'condition' เพื่อทำการตรวจสอบ")
        else:
            word = text.split(";;")[0]
            title = text.split(";;")[1]
    ...
    async def data_del(self, ctx, *, text):
        if ";;" not in text:
            ...
            if value_userid == "user:False":
                ...
                await ctx.send(f"คุณไม่เคยลงคำว่า '{str(title)}' ใน data อยู่แล้ว")
            else:
